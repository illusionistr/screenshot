...
class ClientExportService {
  ...
  static Future<ExportResult> exportAllScreens({
    ...
    if (exportableScreens.isEmpty) {
      return ExportResult(
        ...
        errors: ['No screens with valid screenshots found for ${config.languageCode} + ${config.deviceId}'],
    ...
    try {
      for (int i = 0; i < exportableScreens.length; i++) {
        ...
        try {
          ...
          final exportSize = getExportDimensions(config.deviceId, false); // Always portrait for now
      ...
      return ExportResult(
        ...
        errors: errors,
        finalStatus: errors.isEmpty ? ExportStatus.completed : ExportStatus.failed,
        startedAt: startTime,
        completedAt: DateTime.now(),
        projectName: config.projectName,
        deviceId: config.deviceId,
  ...
  static Future<Uint8List> exportSingleScreen({
    ...
    // 2. Draw device frame with screenshot positioned correctly
    await _drawDeviceFrameWithScreenshot(canvas, screenshot, deviceId, exportSize);
    
    // 3. Draw text overlays
    await _drawTextOverlays(canvas, screenConfig.textConfig, exportSize);
  ...
  static List<Map<String, dynamic>> getExportableScreens(EditorState editorState, ProjectModel project) {
    ...
    for (final screen in editorState.screens) {
      if (screen.assignedScreenshotId != null) {
        ...
        final screenshots = project.getScreenshotsForDevice(
          editorState.selectedDevice, 
          editorState.selectedLanguage
        );
  ...
  static Size getExportDimensions(String deviceId, bool isLandscape) {
    ...
    final dimensions = PlatformDimensions.getDimensionsForDevice(device, isLandscape: isLandscape);
  ...
  static Future<void> _drawBackground(Canvas canvas, ScreenBackground background, Size size) async {
  ...
  static Future<void> _drawDeviceFrameWithScreenshot(Canvas canvas, ScreenshotModel screenshot, String deviceId, Size exportSize) async {
    ...
    if (frameVariant != null && !frameVariant.isGeneric && frameVariant.assetPath != null) {
      await _drawRealDeviceFrame(canvas, screenshot, device, frameVariant.assetPath!, exportSize);
    } else {
  ...
  static Future<void> _drawRealDeviceFrame(Canvas canvas, ScreenshotModel screenshot, DeviceModel device, String assetPath, Size exportSize) async {
    try {
      ...
      if (frameImage != null) {
        ...
        final frameRect = Rect.fromLTWH(frameOffsetX, frameOffsetY, scaledFrameWidth, scaledFrameHeight);
        final frameSrcRect = Rect.fromLTWH(0, 0, frameImage.width.toDouble(), frameImage.height.toDouble());
        canvas.drawImageRect(frameImage, frameSrcRect, frameRect, Paint()..filterQuality = ui.FilterQuality.high);
        
        // Now draw the screenshot in the correct position
        final screenshotImage = await _loadImageFromNetwork(screenshot.storageUrl);
        if (screenshotImage != null) {
          // Calculate screenshot position based on device screen area
          final screenLeft = frameOffsetX + (device.screenPosition.dx * scaleFactorX);
          final screenTop = frameOffsetY + (device.screenPosition.dy * scaleFactorY);
          final screenWidth = device.screenWidth * scaleFactorX;
          final screenHeight = device.screenHeight * scaleFactorY;
          
          final screenRect = Rect.fromLTWH(screenLeft, screenTop, screenWidth, screenHeight);
          final screenshotSrcRect = Rect.fromLTWH(0, 0, screenshotImage.width.toDouble(), screenshotImage.height.toDouble());
          
          // Clip to screen area with rounded corners
          canvas.save();
          canvas.clipRRect(RRect.fromRectAndRadius(screenRect, const Radius.circular(8.0)));
          canvas.drawImageRect(screenshotImage, screenshotSrcRect, screenRect, Paint()..filterQuality = ui.FilterQuality.high);
          canvas.restore();
        }
  ...
  static Future<void> _drawGenericFrameWithScreenshot(Canvas canvas, ScreenshotModel screenshot, Size exportSize) async {
    ...
    final frameTop = exportSize.height * 0.15; // 15% from top for title space + 60% frame + remainder for balance
    ...
    canvas.drawRRect(
      RRect.fromRectAndRadius(frameRect.translate(0, 2), const Radius.circular(borderRadius)),
      shadowPaint,
    );
    ...
    final contentRect = Rect.fromLTWH(contentLeft, contentTop, contentWidth, contentHeight);
    ...
    try {
      ...
      if (screenshotImage != null) {
        canvas.save();
        canvas.clipRRect(RRect.fromRectAndRadius(contentRect, const Radius.circular(innerRadius)));
        ...
        canvas.drawImageRect(screenshotImage, srcRect, dstRect, Paint()..filterQuality = ui.FilterQuality.high);
        canvas.restore();
        return;
      }
    } catch (e) {
  ...
  static Future<void> _drawTextOverlays(Canvas canvas, ScreenTextConfig textConfig, Size exportSize) async {
      ...
      final textPainter = TextPainter(
        text: TextSpan(
          ...
          style: TextStyle(
            ...
            fontSize: exportFontSize.clamp(16.0, 200.0), // Reasonable bounds for export
